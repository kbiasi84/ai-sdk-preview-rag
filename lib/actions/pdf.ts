...
export async function processPdfFile(
  ...
  try {
    console.log(`Iniciando processamento do PDF: ${file.name} (${file.size} bytes)`);
    
    const arrayBuffer = await file.arrayBuffer();
    console.log(`ArrayBuffer criado com tamanho: ${arrayBuffer.byteLength} bytes`);
    ...
    pdfText += "O conteúdo deste PDF foi processado para indexação e pesquisa, mas o texto extraído diretamente pode não estar disponível na íntegra devido a limitações do processo de extração.\n\n";
    ...
    let errorMessage = "Erro ao processar o arquivo PDF. Verifique se o arquivo é válido.";
    ...
    if (error instanceof Error) {
      ...
      if (error.message.includes("password")) {
        errorMessage = "O PDF está protegido por senha e não pode ser processado.";
      } else if (error.message.includes("corrupt") || error.message.includes("invalid")) {
        errorMessage = "O arquivo PDF parece estar corrompido ou inválido.";
      } else if (error.message.includes("memory")) {
        errorMessage = "O PDF é muito grande para ser processado. Tente um arquivo menor.";
      }
    }
...
export async function uploadPdf(formData: FormData) {
  try {
    ...
    if (!file.name.toLowerCase().endsWith(".pdf")) {
      console.error(`Tipo de arquivo inválido: ${file.type}, nome: ${file.name}`);
      return {
        success: false,
